...
class GroupingRelation(conn: Connection,
                       relation: Relation,
                       disafuse: AggregateFunctionUseState,
                       discriminator: Vector[ValueResult],
                       filtafuse: AggregateFunctionUseState,
                       filter: Option[LogicalResult],
                       colafuse: AggregateFunctionUseState,
                       columns: Vector[ValueResult])
    extends AbstractRelation {

  val metadata = new Metadata(columns map (c => SimpleColumn(c.table, c.heading, c.typ)))
